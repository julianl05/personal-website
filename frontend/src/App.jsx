...
function App() {
  ...
  const [expButtonText, setExpButtonText] = useState('Hide')
  const [displayProj, setDisplayProj] = useState('block')
  const [projButtonText, setProjButtonText] = useState('Hide')
  
  function toggleExp() {
    if (displayExp === 'none') {
      setDisplayExp('block')
      setExpButtonText('Hide')
    } else {
      setDisplayExp('none')
      setExpButtonText('Show')
    }
  }

  function toggleProj() {
    if (displayProj === 'none') {
      setDisplayProj('block')
      setProjButtonText('Hide')
    }
    else {
      setDisplayProj('none')
      setProjButtonText('Show')
    }
  }
  ...
  return (
    <>
      ...
      <div id="exp">
        <h2>
          <button className="toggler" onClick={toggleExp}>{expButtonText}</button>
          Experiences
        </h2>
        <div style={{display: displayExp}}>
      ...
      <div id="projects">
        <h2>
          <button className="toggler" onClick={toggleProj}>{projButtonText}</button>
          Projects
        </h2>
        <div style={{display: displayProj}}>
